...
int lcd_write_cmd(uint8 cmd)
{
	...
	if(retval < 0){
		SYS_TRACE(LCD_DRIVER_DEBUG, ("lcd_write_cmd:lcd_write_cmd timeout\r\n"));
		return -1;
	}
...
int lcd_write_data(uint8 data)
{
	...
	retval = lcd_check_busy();
	if(retval < 0){
		SYS_TRACE(LCD_DRIVER_DEBUG, ("lcd_write_data timeout\r\n"));
		return -1;
	}
...
int lcd_init(void)
{
	...
	SYS_TRACE(LCD_DRIVER_DEBUG, ("lcd init done\r\n"));
	return 0;
}
