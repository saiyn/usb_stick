...
int fn1(struct cmd_tbl_s *cmdtp, int argc, char * const argv[])
{
     ...
     unsigned long len = (unsigned long)Load$$INIT_SECTION$$Length;
	   ...
	   SYS_TRACE(SYS_DEBUG_ON, ("\r\n-------------CMD List Table------------------\r\n"));
	   for(i=0;i<len;index++)
     {
         pCmd = (cmd_tbl_t*)(base+i);
         if(pCmd->usage){
            SYS_TRACE(SYS_DEBUG_ON, ("%d.%s          -%s\r\n", index,pCmd->name,pCmd->usage));
				 }
...
static cmd_err_t cmd_process(int argc, char * const argv[])
{
   ...
   cmd_tbl_t* pCmd;
	 ...
	 if(!pCmd){
		 SYS_TRACE(SYS_DEBUG_ON, ("cmd_process:\r\nUnknown cmd '%s'\r\n", argv[0]));
		 return CMD_UNKNOWN;
	 }
	 
	 /* found - check max args */
	 if(pCmd->maxargs < argc){
		 SYS_TRACE(SYS_DEBUG_ON, ("cmd_process:Too many argv\r\n"));
...
static int parse_line(char *line, char *argv[])
{
	 ...
	 SYS_TRACE(SYS_DEBUG_ON, ("** Too many args (max. %d) **\r\n", CONFIG_SYS_MAXARGS));
...
cmd_err_t osal_run_command(char *cmd)
{
	 ...
	 if((argc = parse_line(cmd, argv)) == 0){
		 SYS_TRACE(SYS_DEBUG_ON, ("no command at all\r\n"));
		 return CMD_UNKNOWN;
	 }
